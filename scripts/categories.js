...
var pagination = document.getElementsByClassName('pagination');
    container.onclick=function(e){
      ...
      if (val == 'prev') {
        var counter = x;
       prev.href="?route=category/view&page="+((counter==1)?Number(counter) : (Number(counter)-1));
      }else if(val == 'next'){
        counter = y;
        next.href="?route=category/view&page="+(Number(counter)+1);
      }
    ...
    function set(){
        ...
        if((Number(current) /2) <=1){
          for(var i =0 ;i<2;i++){
            ...
            pikin.href ="?route=category/view&page="+(page+i);
