...
const paginator ={
  createPageLink(container,page,nextPage,num)
  {
    for(let i=0;i<num;i++){
      ...
      pageNum.href ="?route=article/list&page="+(page+i);
  ...
  */
  ...
  getPage(e)//renders the previous or next page
  {
    ...
    switch(value){
      case 'prev':
        View.previousPage.href="?route=article/list&page="+((x==1)?Number(x) : (Number(x)-1));
        break;
      case 'next':
        View.nextPage.href="?route=article/list&page="+(Number(y)+1);
        break;
    }
  },
};
